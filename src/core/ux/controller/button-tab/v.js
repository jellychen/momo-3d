...
const tagName = 'x-button-tab';

/**
 * Tab 按钮
 */
export default class ButtonTab extends Element {
    /**
     * 回调函数
     */
    ...
    onItemSelected(item) {
        ...
        // 数据回调
        if (isFunction(this.#on_item_changed_callback)) {
            this.#on_item_changed_callback(item);
        }
    }
}

CustomElementRegister(tagName, ButtonTab);
